...
if not os.path.exists('plot/RdgeRegression'):
    os.mkdir('plot/RdgeRegression')

def RidgeRegression(train, test):
    train_identifier = []
    ...
    ridge_regression.fit(train_dataset, train_labels)
    predictions = ridge_regression.predict(test_dataset)
    score = ridge_regression.score(train_dataset, train_labels)
    ...
    #cross-validation (cv):
    cv_score = cross_val_score(ridge_regression, train_dataset, train_labels, cv=20)
    ...
    mean_results['Ridge Regression'] = lr_mean
    std_results['Ridge Regression'] = lr_std
    ...
    plt.grid(True)
    plt.savefig('plot/RdgeRegression/plot1.pdf')
    ...
    plt.grid(True)
    plt.plot(lims, lims)
    plt.savefig('plot/RdgeRegression/plot2.pdf')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    _ = plt.ylabel("Count")
    plt.savefig('plot/RdgeRegression/plot3.pdf')
    plt.show(block=False)
    plt.pause(5)
    plt.close()

    unknow_data = test[predictors]
    unknow_predictions = ridge_regression.predict(unknow_data)
    ...
    plt.ylabel('Item Outlet Sales')
    plt.grid(True)
    plt.savefig('plot/RdgeRegression/plot4.pdf')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
