...
if not os.path.exists('plot/DecisionTree'):
    os.mkdir('plot/DecisionTree')
...
def DecisionTree(train, test):    
    ...
    decisionTree.fit(train_dataset, train_labels)
    predictions = decisionTree.predict(test_dataset)
    score = decisionTree.score(train_dataset, train_labels)
    ...
    #cross-validation (cv):
    cv_score = cross_val_score(decisionTree, train_dataset, train_labels, cv=20)
    #print(cv_score)
    ...
    mean_results['Decision Tree'] = lr_mean
    std_results['Decision Tree'] = lr_std
    ...
    plt.grid(True)
    plt.savefig('plot/DecisionTree/plot1.pdf')
    ...
    plt.grid(True)
    plt.plot(lims, lims)
    plt.savefig('plot/DecisionTree/plot2.pdf')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    _ = plt.ylabel("Count")
    plt.savefig('plot/DecisionTree/plot3.pdf')
    plt.show(block=False)
    plt.pause(5)
    plt.close()

    unknow_data = test[predictors]
    unknow_predictions = decisionTree.predict(unknow_data)
    ...
    plt.ylabel('Item Outlet Sales')
    plt.grid(True)
    plt.savefig('plot/DecisionTree/plot4.pdf')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
